...
export function MouseCursor() {
  ...
  return (
    <>
      ...
      <div
        ref={followerRef}
        className={`fixed w-8 h-8 rounded-full border border-white/30 bg-white/10 backdrop-blur-sm pointer-events-none z-[9999] transition-opacity duration-300 ${
      ...
      <div
        ref={dotRef}
        className={`fixed w-1 h-1 rounded-full bg-white pointer-events-none z-[9999] transition-opacity duration-300 ${
          isVisible ? 'opacity-100' : 'opacity-0'
        }`}
      />
    </>
